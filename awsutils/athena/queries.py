...
def execute(
    ...
    logging.info(
        ...
        query, query_execution["QueryExecutionId"]
    )
...
def get_query_result(ath_client: object, execution_id:str):
    response = ath_client.get_query_execution(
        QueryExecutionId=execution_id)

    logging.info(
        f'Query %s com o estado %s.', execution_id,
...
def read_response(query_execution_response: dict):
    ...
    result_dict['tempo_execucao_query_ms'] = statistics['TotalExecutionTimeInMillis']
